...
/**
 * updates all possible movements for queen based on its king position and board state
 * @param king
 * @param board
 ...
 * @param changeLetPos
 */
void Queen::getMove(King * king, Board * board, vector<FieldToMove> & fullRangeOfMoves, const int & changeNumPos, const int & changeLetPos) {
...
/**
 * returns all possible moves for queen based on current state of board and its king position
 * @param king
 * @param board
 * @return
 */
vector < FieldToMove > Queen::getFullRangeOfMoves(King * king, Board * board) {
    vector < FieldToMove > fullRangeOfMoves;
    int directions[2] = {-1,1};
    vector<pair<int,int>> allDirections;
    for (int i : directions) { allDirections.push_back(make_pair(i,0)); allDirections.push_back(make_pair(0,i)); }
